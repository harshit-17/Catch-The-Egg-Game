...
void start_screen(int i, int j) //2
{
    int k;
    char cat[4] = "EGG";
    char orr[9] = "Catching";
...
void cloud1() //4
{
    ...
    {
        theta = i * PI * i / 180;
        glVertex2f(100 + 25 * cos(theta) , 590 + 25 * sin(theta));
    }
    glEnd();
    ...
    {
        theta = i * PI * i / 180;
        glVertex2f(130 + 25 * cos(theta) , 580 + 25 * sin(theta));
    }
    glEnd();
    ...
    {
        theta = i * PI * i / 180;
        glVertex2f(140 + 25 * cos(theta) , 600 + 25 * sin(theta));
    }
    glEnd();
    ...
    {
        theta = i * PI * i / 180;
        glVertex2f(170 + 25 * cos(theta) , 590 + 25 * sin(theta) );
    }
...
void cloud2() //5
{
    ...
    {
        theta = i * PI * i / 180;
        glVertex2f(420 + 23 * cos(theta), 540 + 23 * sin(theta) );
    }
    ...
    {
        theta = i * PI * i / 180;
        glVertex2f(455 + 23 * cos(theta) , 550 + 23 * sin(theta) );
    }
    ...
    {
        theta = i * PI * i / 180;
        glVertex2f(455 + 23 * cos(theta) , 530 + 23 * sin(theta) );
    }
    ...
    {
        theta = i * PI * i / 180;
        glVertex2f(490 + 23 * cos(theta) , 540 + 23 * sin(theta) );
    }
...
void duck(int i, int j) //11
{
    int h;
    glColor3f(1.0, 1.0, 0.0);
    glBegin(GL_POLYGON); // Duck Figure
    ...
    for (h = 0; h < 13; h += 4)  // Wings on duck
    {
        glBegin(GL_LINES);
        glColor3f(0.7, 0.4, 0);
...
void display(void) //15
{
    ...
    if (missed_eggs >= 10)
    {
        ...
        glRasterPos2i((a / 2) - 30, b / 2);
        glColor3f(0, 1, 0);
        for (int k = 0; k < 10; k++)
            glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, game[k]);
    }
    if (s >= 1)
    {
        glColor3f(0, 0, 1);
...
void basket_set(int a, int b) //16
{
    basket_x = a;
    basket_y = b;
    glutPostRedisplay();
...
void keys(unsigned char key, int x, int y) //17
{
    if (key == 'q' || key == 'Q')
    {
        printf("\n\n\n\t\tQUIT BY PLAYER\n\n");
